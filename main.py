...
def initiate_delete(message):
    ...
    # If the user has no coins in their portfolio, send a message
    if not rows:
        bot.send_message(message.chat.id, "You have no coins in your portfolio. Add coins using the /add command.")
        return

    # Get unique symbols from the user's portfolio
    symbols = set(row[0] for row in rows)

    # Send the list of symbols to the user
...
def delete_coin(message):
    ...
    if len(rows) == 1:
        # If there's only one transaction, ask for confirmation before deleting it
        ...
        # If there are multiple transactions, list them and ask the user to choose
...
def confirm_delete(message, row, confirm_keyboard=None):
    ...
    if message.text.lower() == 'confirm':
        id = row[0]  # extract the id value from the row tuple
